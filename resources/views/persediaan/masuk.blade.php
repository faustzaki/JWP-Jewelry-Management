...
<div class="space-y-6">
 ...
 <h1 class="text-2xl font-heading">Transaksi Barang Masuk</h1>
 </div>
 ...
 <h3 class="text-lg font-heading mb-4">Input Barang Masuk</h3>
 <form action="{{ route('persediaan.masuk.store') }}" method="POST" class="space-y-4">
 @csrf
 <!-- Form Group: Tanggal -->
 <div class="space-y-1.5">
 <label class="text-sm font-medium text-neutral-text-muted">Tanggal Masuk</label>
 <input type="date" name="tanggal_masuk" required class="w-full border border-neutral-border bg-transparent rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white transition-shadow dark:[color-scheme:dark]" value="{{ date('Y-m-d') }}" />
 </div>
 ...
 <!-- Form Group: Jumlah -->
 <div class="space-y-1.5">
 <label class="text-sm font-medium text-neutral-text-muted">Jumlah Masuk</label>
 <input type="number" name="jumlah_masuk" required min="1" class="w-full border border-neutral-border bg-transparent rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white transition-shadow" placeholder="0" />
 </div>

 <!-- Form Group: Keterangan -->
 <div class="space-y-1.5">
 <label class="text-sm font-medium text-neutral-text-muted">Keterangan / Supplier</label>
 <textarea name="keterangan" class="w-full border border-neutral-border bg-transparent rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white transition-shadow" rows="2" placeholder="Catatan..."></textarea>
 </div>
 ...
 <h3 class="text-lg font-heading">Riwayat Barang Masuk</h3>
 ...
 <td class="py-3 px-6 text-neutral-text-muted">
      {{ \Carbon\Carbon::parse($item->tanggal_masuk)->format('d-m-Y') }} <span class="text-xs text-neutral-text-muted/80 font-mono">{{ $item->created_at->format('H:i:s') }}</span>
  </td>
 <td class="py-3 px-6">{{ $item->barang->nama_barang }}</td>
 <td class="py-3 px-6 text-green-600 dark:text-green-400 text-right">+{{ $item->jumlah_masuk }}</td>
 <td class="py-3 px-6 text-neutral-text-muted">{{ $item->keterangan ?? '-' }}</td>
 </tr>
 @empty
 <tr>
    <td colspan="4" class="text-center py-12 text-neutral-text-muted">Belum ada data transaksi barang masuk.</td>
 </tr>
 @endforelse
 </tbody>
 </table>
